...
Shader::Shader()
{
	...
	psoDesc.pRootSignature = ClientApp::GetInstance()->GetRootSignature().Get();
	...
	DX::ThrowIfFailed(ClientApp::GetInstance()->GetD3DDevice()->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&m_pipelineState)));
}
...
LineShader::LineShader()
{
	...
	HRESULT hr{ E_FAIL };
	hr = D3DCompileFromFile(L"Default.hlsl", NULL, D3D_COMPILE_STANDARD_FILE_INCLUDE, "VS", "vs_5_1", compileFlags, 0, &vertexShader, &error);
	hr |= D3DCompileFromFile(L"Line.hlsl", NULL, D3D_COMPILE_STANDARD_FILE_INCLUDE, "GS", "gs_5_1", compileFlags, 0, &geometryShader, &error);
	hr |= D3DCompileFromFile(L"Line.hlsl", NULL, D3D_COMPILE_STANDARD_FILE_INCLUDE, "PS", "ps_5_1", compileFlags, 0, &pixelShader, &error);
#ifdef _DEBUG
	if (FAILED(hr))
	{
		OutputDebugStringA(reinterpret_cast<char*>(error->GetBufferPointer()));
