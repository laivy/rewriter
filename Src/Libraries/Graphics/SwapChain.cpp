...
namespace Graphics::D3D
{
	...
	void SwapChain::Begin3D()
	{
		...
		auto barriers{ CD3DX12_RESOURCE_BARRIER::Transition(m_frameResources[m_frameIndex].backBuffer.Get(), D3D12_RESOURCE_STATE_PRESENT, D3D12_RESOURCE_STATE_RENDER_TARGET) };
		g_commandList->ResourceBarrier(1, &barriers);
	...
	{
		auto barrier{ CD3DX12_RESOURCE_BARRIER::Transition(renderTarget->GetResource().Get(), D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_RENDER_TARGET) };
	...
	{
		auto barrier{ CD3DX12_RESOURCE_BARRIER::Transition(g_renderTargets.back()->GetResource().Get(), D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_COMMON) };
		g_commandList->ResourceBarrier(1, &barrier);
