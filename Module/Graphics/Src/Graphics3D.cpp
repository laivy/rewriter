...
namespace Graphics::D3D
{
	...
	{
		RECT rect{};
		if (!::GetClientRect(static_cast<HWND>(g_hWnd), &rect))
			return false;
	...
	static std::unique_ptr<IndexBuffer> CreateIndexBuffer(const std::vector<int>& indices)
	{
		...
		auto barrier{ CD3DX12_RESOURCE_BARRIER::Transition(buffer.Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_INDEX_BUFFER) };
		g_commandList->ResourceBarrier(1, &barrier);
	...
	/*
	std::shared_ptr<Resource::Texture> LoadTexture(std::span<std::byte> binary)
	{
	...
	*/
	...
	std::shared_ptr<RenderTarget> CreateRenderTarget(std::uint32_t width, std::uint32_t height)
	{
	...
	void Render(const std::shared_ptr<Resource::Model>& model)
	{
