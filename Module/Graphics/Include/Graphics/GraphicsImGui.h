...
namespace Graphics::ImGui
{
	...
	GRAPHICS_API std::shared_ptr<Texture> LoadTexture(std::span<char> binary);
	GRAPHICS_API ImTextureRef GetTextureRef(const std::shared_ptr<Texture>& texture);
	GRAPHICS_API ImVec2 GetTextureSize(const Resource::Sprite& sprite);
	GRAPHICS_API ImVec2 GetTextureSize(const std::shared_ptr<Texture>& texture);
	GRAPHICS_API void Image(const Resource::Sprite& sprite, const ImVec2& size = ImVec2{ 0, 0 }, const ImVec2& uv0 = ImVec2{ 0, 0 }, const ImVec2& uv1 = ImVec2{ 1, 1 });
